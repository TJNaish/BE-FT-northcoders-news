      ...
      it("/:topic_slug/articles GET responds with 200 and a list of articles with the passed topic", () => {
        return request
          ...
          .then(res => {
            ...
            expect(res.body.articles[0].belongs_to).to.eql(
              res.body.articles[1].belongs_to
            );
          });
      });
    ...
    describe("articles", () => {
      ...
      it("/:article_id/comments GET responds with all comments for one article", () => {
        return request
          ...
          .then(res => {
            ...
            expect(res.body.comments[0].belongs_to).to.eql(
              res.body.comments[1].belongs_to
            );
          });
      });
      ...
      it("/:article_id/comments GET responds with 400 when article id is not valid", () => {
        return request
          ...
          .then(res => {
            expect(res.body.message).to.equal(
              `Cast to ObjectId failed for value "asdfzg" at path "_id" for model "articles"`
            );
          });
      });
      it("GET a single article by id", () => {
        return request
          ...
          .expect(200)
          .then(res => {
            expect(res.body.foundArticle).to.contain.keys(
              "title",
              "belongs_to",
              "created_by",
              "body",
      ...
      it("/:article_id/ GET responds with 404 when article id is not found", () => {
        return request
          ...
          .then(res => {
            expect(res.body.message).to.equal(
              "Article with ID 5b51e71d2e98242c21cc3e39 not found"
            );
          });
      });
      ...
      it("PUT a vote counter up on a single article", () => {
        ...
        request
          ...
          .expect(200)
          .then(res => {
            expect(res.body.foundArticle.votes).to.equal(
              articleDocs[1].votes + 2
            );
          });
      });
    ...
    describe("users", () => {
      it("/:username GET returns a JSON object with profile data for the user", () => {
        return request
          ...
          .expect(200)
          .then(res => {
            expect(res.body.foundUser).to.contain.keys(
              "username",
              "name",
              "avatar_url"
            );
      ...
      it("/:username GET responds with 404 when username is not found", () => {
        return request
          ...
          .then(res => {
            expect(res.body.message).to.equal(
              "Username sergeantwuffles not found"
            );
          });
      });
    });
    describe("comments", () => {
      it("PUT a vote counter up on a single comment", () => {
        ...
        request
          ...
          .expect(200)
          .then(res => {
            expect(res.body.updatedVote.votes).to.equal(
              commentDocs[1].votes + 2
            );
          });
      });
      ...
      it("PUT counter returns 404 when the comment ID is not found", () => {
        return request
          ...
          .then(res => {
            expect(res.body.message).to.equal(
              "Comment with ID 5b51e71d2e98242c21cc3e39 not found"
            );
          });
      });
      it("PUT counter returns 400 when the comment ID is not valid", () => {
        return request
          ...
          .then(res => {
            expect(res.body.message).to.equal(
              `Cast to ObjectId failed for value "steve" at path "_id" for model "comments"`
            );
          });
      });
