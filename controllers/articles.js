...
const getArticleComments = (req, res, next) => {
  Article.findOne({ _id: req.params.article_id })
    .then(foundArticle => {
      Comment.find({ belongs_to: foundArticle._id }).then(comments => {
  ...
  Article.findOne({ _id: req.params.article_id })
    .then(foundArticle => {
      foundArticle === null
        ? next({
            status: 404,
            message: `Article with ID ${req.params.article_id} not found`
          })
...
const postNewComment = (req, res, next) => {
  ...
  newComment
    ...
    .then(comment => {
      res
        .status(201)
        .send({ comment, message: "Your comment has been added." });
    })
    .catch(next);
};
